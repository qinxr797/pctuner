...
不玩 FPS 的人打开会觉得这工具跟自己没关系。

v3.0 把「三个 FPS 游戏」抽象成**八种使用场景**：

> 竞技射击（CS2 / 无畏契约 / 三角洲）· 大型单机 3A · 网游挂机 · 轻度游戏 / 模拟器 · 不玩游戏（办公上网）· 老机器救急

![按使用场景给结论](docs/screenshot-scenarios.png)

**关键在于：同一项对不同的人，结论经常是相反的。**

上图是「关闭 SysMain」这一项的真实结论：网游玩家「推荐」、老机器用户「慎用」
（机械盘靠它预读，关了开程序更慢）、CS2 玩家「中性」。
只给一个笼统的「建议开启」是不负责任的。

全部 55 个优化项 × 8 种场景的结论都写全了，没有一条是靠模板凑的。

首页预设也重做了，现在第一排是「**按用途选**」：

| 预设 | 给谁 | 思路 |
|---|---|---|
| 大型单机 3A | 黑神话、艾尔登法环这类 | 不卡顿、读图快，**不关全屏优化**（这点和 FPS 相反） |
| 网游 / 挂机 / 多开 | 梦幻、剑网3、原神 | 重点在网络延迟，**不动 CPU 时间片**（多开会更卡） |
| ★ 不玩游戏 | 办公上网刷视频 | 全是每天能感觉到的：界面响应、开机、浏览器、广告 |
| 老机器救急 | 内存小 / 机械盘 | **刻意避开那些在老机器上帮倒忙的**（关内存压缩、关 SysMain 等） |
...
界面动画用的是 WPF 原生 Storyboard，**没有引入任何第三方 UI 库** —— 那会毁掉「解压双击就能跑、零依赖、两百 KB」这个优点。动画只动 Opacity 和 Transform（走 GPU 合成，不触发重新布局），时长压在 120~220ms。

**可以整体关掉**：这个工具本来就服务配置吃紧的机器，动画不能反过来变成负担。开关在「个性化」页，关了之后所有切换瞬间完成，功能一模一样。
